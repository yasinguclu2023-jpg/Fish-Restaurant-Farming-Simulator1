    ...
    public partial class OutlineFxFeature
    {
        ...
        private class Pass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                // render child renderer entries (from parent Outline components)
                foreach (var entry in _renderEntries)
                {
                    if (entry.renderer == null)
                        continue;

                    cmd.SetGlobalTexture(s_MainTex, entry.renderer.sharedMaterial.mainTexture);
                    cmd.SetGlobalColor(s_Color, entry.color);
                    cmd.DrawRenderer(entry.renderer, _owner._outlineMat, 0, 0);
                }
